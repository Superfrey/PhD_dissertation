...
table_mas %>%
    ...
    kableExtra::footnote(general="Categorical variables: N (%); Continous variables: Median (IQT:25th-75th). Table are adapted from supplementary material in Appendix Study I.")
```
...
![](images/web/figure3_linear_plot.png)
...
table_add_pro[-nrow(table_add_pro), ] %>% 
    ...
    font_size = 10) %>% 
    kableExtra::landscape()%>% 
    kableExtra::footnote(general= "Categorical variables: N (%); Continous variables: Median (IQT:25th-75th). Table are based on data from Study II.")
```
...
![](images/web/addition_pro_hrv_ir_mace.png)
...
::: {#fig-ADD_PRO_risk_by_hour fig-cap="**Diurnal heart rate variability and heart rate association with major adverse cardiovascular events, heart failure, and all-cause mortality risk**" fig-align="center"}
![](images/web/figure_ADD_PRO_risk_by_hour.png)
...
#| warning: false
#| message: false
#| tbl-cap: "Study characteristics in The CANCAN Study (Study III) by CAN status"
#| label: tbl-can
...
table_can[-nrow(table_can), ] %>% 
    ...
    font_size = 10) %>% 
    kableExtra::landscape()%>% 
    kableExtra::footnote(general= "Categorical variables: N (%); Continous variables: Median (IQT:25th-75th). Table are adapted from Table 1 in Appendix Study III.")
...
![](images/web/heart_failure_distribution_can_figure_2_2.png){width="85%"}
